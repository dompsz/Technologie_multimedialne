...
// Zabezpieczenie - tylko dla użytkownika o loginie 'admin'
if (!isset($_SESSION['lab14_user_id']) || $_SESSION['lab14_username'] !== 'admin') {
    die("Brak uprawnień administratora. Musisz być zalogowany jako 'admin'.");
}

// 1. Pobierz wszystkich użytkowników (oprócz admina)
$stmt_users = $conn->query("SELECT id, username, created_at FROM users WHERE username != 'admin' ORDER BY created_at DESC");
